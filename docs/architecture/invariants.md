...
| --- | --- | --- | --- |
| `voice.control-queue-capacity` [fact:voice.control-queue-capacity] | 16 events | `voice_controller.rs::CONTROL_QUEUE_CAPACITY` | 控制事件积压超过上限时失败关闭 |
| `voice.audio-queue-capacity` [fact:voice.audio-queue-capacity] | 32 chunks | `voice_controller.rs::AUDIO_QUEUE_CAPACITY` | 不允许用无界内存掩盖 provider 过慢 |
| `voice.recording-deadline-seconds` [fact:voice.recording-deadline-seconds] | 120 秒 | `voice_controller.rs::MAX_RECORDING_SECS` | 单次录音必须自动 finalize |
| `voice.stream-chunk-milliseconds` [fact:voice.stream-chunk-milliseconds] | 200ms | `voice_controller.rs::STREAM_CHUNK_MS` | 决定音频队列可缓冲的时间尺度 |
| `voice.final-transcript-timeout-seconds` [fact:voice.final-transcript-timeout-seconds] | 25 秒 | `voice_controller.rs::FINAL_TRANSCRIPT_TIMEOUT_SECS` | 有 preview 时等待最终文本的上限 |
| `voice.empty-transcript-timeout-milliseconds` [fact:voice.empty-transcript-timeout-milliseconds] | 800ms | `voice_controller.rs::EMPTY_TRANSCRIPT_TIMEOUT_MS` | 无 preview 时快速结束空输入 |
