...
`用户目标`和`验收场景`优先使用用户可观察结果、失败边界和允许的副作用，不把当前 API、句柄类型、类名、线程模型、动态编号或固定供应商格式名单写成产品契约。写入一个技术名词前先问：如果未来用另一种机制仍能提供相同行为，这个名词是否还必须存在？如果答案是否定的，把它移到 ADR、当前实现说明或绑定 revision 的证据。

- 动态 ID、错误码样例和第三方私有格式名称通常是诊断证据，不是稳定验收条件。
- 数据完整性契约描述“能否成为应用自有且可独立重建的数据”；具体用 `HGLOBAL`、文件、流还是平台对象实现由 ADR 和源码负责。
- 测试名称不得把未经验证的实现假设包装成产品结论。例如“未知格式必须失败”只有在“未知”本身就是确认的风险证明时才成立；否则测试应围绕可观察能力分类。
- Dossier 的`当前实现入口`和`验证状态`可以简述机制，但不能反向覆盖用户契约。
...
当一份 Dossier 的验收切片覆盖多个可独立失败、独立发布或独立验收的用户生命周期，导致任一局部修改都把大量无关切片标为可能过期时，应提出拆分，而不是继续增加组件和验收项。拆分以用户失败边界和验证闭环为依据，不按当前类或目录机械切割；迁移时保留旧验收 ID 的映射，避免为“整理文档”制造验证证据丢失。拆分本身是一次有成本的契约迁移，应由收口者在 clean revision 上执行，不在缺陷修复中顺手重排。
...
证据按事件写入，而不是每个任务都写。只有以下情况需要新增或更新证据：准备升级验证状态、真实目标环境/人工质量/故障注入结果会改变完成判断、现有证据被新源码影响，或发布工件需要追溯。普通单元测试与 CI 结果保留在 commit/CI 或可选 PR；只有它承担某个验收切片的长期证明时才进入 Dossier。

进入 Dossier 的开发证据记录 source revision、worktree 状态、变更路径、环境和日期，并明确它证明的能力、范围和限制。发布级证据再增加 build ID 和 artifact SHA-256。

“暂时未复现”“看起来正常”或一次开发机冒烟是有价值的反馈信号，但缺少精确 revision、工件身份、场景矩阵、重复次数或判定方法时，不创建会参与状态计算的正式 evidence 对象。它可以进入`验证状态`的当前说明、任务摘要或可选 PR，并必须同时写出限制；只有信息补齐后才升级为结构化证据。反过来，任何真实崩溃、数据损坏或不可逆副作用即使缺少完整根因，也足以否定对应范围的完成声明，应记录为失败证据。
