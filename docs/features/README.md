...
“暂时未复现”“看起来正常”或一次开发机冒烟是有价值的反馈信号，但缺少精确 revision、工件身份、场景矩阵、重复次数或判定方法时，不创建会参与状态计算的正式 evidence 对象。它可以进入`验证状态`的当前说明或 PR，并必须同时写出限制；只有信息补齐后才升级为结构化证据。反过来，任何真实崩溃、数据损坏或不可逆副作用即使缺少完整根因，也足以否定对应范围的完成声明，应记录为失败证据。
