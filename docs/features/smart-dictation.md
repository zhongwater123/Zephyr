...
| `AC-SD-14` | 首页设置栏的“输入效果”末尾显示唯一的智能润色入口；用户可选择 `Fast`、轻微整理、自然表达或理清重点，默认自然表达。`Fast` 显示“快速响应，仅识别原话”；其余文案说明允许的整理结果，不暴露 Prompt、模型、画像、路由或“介入强度”等专业术语。选择后自动保存并在重启后保持。控件表现为四档磁吸轨道：拖拽位置在视觉上连续，松手后吸附到四个真实档位，且一次拖拽只提交一次配置；档位分界带迟滞，避免停在边界附近时名称、描述和轨道效果反复抖动。轨道两端固定英文 `Faster` / `Smarter`。轨道内像素能量随档位增强（`Fast` 不点亮，最高档铺满轨道），滑块右侧不点亮，因此点亮跨度本身就是当前档位的读数；密度与强度按档位阶跃，不表达档位之间的中间强度。当前档位名称与一句话描述在固定高度窗口内做方向性滚动切换，快速跨档时 latest-wins 收敛到最新档位，不排队播放中间动画，切换不导致侧栏上下跳动。保存期间保留用户已选择的乐观位置并以节点光晕表达保存中，保存失败回弹到上一个已保存档位且不产生第二次提交。读屏只播报已提交档位，像素层对辅助技术隐藏；`prefers-reduced-motion: reduce` 下停用弹性、滚动与动态噪声但保留静态密度与当前档位反馈；窗口不可见时暂停非必要动画 | 前端自动化 + 目标用户可用性观察 + Windows WebView2 视觉/键盘可访问性 + 重启持久化 |
