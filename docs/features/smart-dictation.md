...
| `AC-SD-14` | 首页设置栏的“输入效果”末尾显示唯一的智能润色入口；用户可选择极速模式、轻微整理、自然表达或理清重点，默认自然表达，选择后自动保存并在重启后保持。**模块只显示当前档位**：标题行内并排呈现档位名与一句话说明，不再常驻列出四个档位名，也不再显示引导语；文案不暴露 Prompt、模型、画像、路由或“介入强度”等专业术语。控件表现为四档磁吸轨道：拖拽位置在视觉上连续，松手后吸附到四个真实档位，且一次拖拽只提交一次配置；单击轨道直接吸附到目标档位，不产生二次位移；档位分界带迟滞，避免停在边界附近时名称、描述和轨道效果反复抖动。轨道两端固定英文 `Faster` / `Smarter`。三个 LLM 档位的像素能量自滑块**向左**扩散并随档位增强（最高档铺满轨道），滑块右侧不点亮，因此点亮跨度本身就是当前档位的读数；密度、强度与运动速率按档位阶跃，不表达档位之间的中间强度。极速模式作为**独立模式**而非最低强度档，拥有自己的视觉语言：自滑块**向右**的短促蓝色羽流、独立的蓝色档位名、以及最快的喷射与最短的尾迹；其点亮面积不得超过轻微整理，以免最低档读成最强档。当前档位名与说明位于同一条固定高度的滚动纸带上，升档时纸带向下滚动（爬升感）、降档相反，切换带方向性动态模糊，快速跨档时 latest-wins 收敛到最新档位，不排队播放中间动画，切换不导致侧栏上下跳动。保存对用户不可见：保留乐观位置、不禁用控件、不阻塞下一次操作，保存失败回弹到上一个已保存档位且不产生第二次提交。读屏只播报已提交档位，像素层对辅助技术隐藏；`prefers-reduced-motion: reduce` 下停用滚动与动态噪声但保留静态密度与当前档位反馈；窗口不可见时暂停非必要动画 | 前端自动化 + 目标用户可用性观察 + Windows WebView2 视觉/键盘可访问性 + 重启持久化 |
...
- 2026-08-31：用户确认智能润色控件采用四档磁吸像素能量轨道与滚动式档位响应动画：顶部为“模块名称 + 当前档位”的紧凑结构，轨道两端固定英文 `Faster` / `Smarter`，仍然只有四个真实输出档位。拖拽视觉连续但松手磁吸到离散档位，一次拖拽只提交一次配置；越接近 `Smarter` 像素越密、越亮、运动越活跃，`Fast` 不点亮轨道而最高档铺满轨道，滑块右侧不点亮。该轮只调整设置界面及其交互反馈，不改变四档后端语义、Prompt 或模型处理策略，因此不新增 ADR；`CI-SD-02` 关于“连续视觉不得在后端静默压回粗粒度档位”的约束继续有效，本次实现通过“仅点亮跨度连续、密度与强度按档位阶跃、提交值恒为 0–3”来满足该约束。
...
- 2026-09-01：用户在真实 Windows 窗口中逐轮评审智能润色控件后，确认以下界面契约修订。**四档文案改为**：极速模式／适合高频短对话、轻微整理／Zephyr轻微整理、自然表达／让表达更顺，合适时自动整理要点。、理清重点／深度整理优化；原 `Fast` 名称与“快速响应，仅识别原话”文案不再使用。**信息密度**：删除“说完后，希望得到怎样的文字？”与“选择更快直出…”两句引导语，取消常驻的四档标签行，档位名与说明合并到标题行内的同一条滚动纸带，模块只显示当前档位。**极速模式确立为独立模式的视觉表达**：自滑块向右的短促蓝色羽流、蓝色档位名、最快喷射与最短尾迹，点亮面积不超过轻微整理——这是对 `ASM-SD-11`（Fast 是 ASR 直出模式而非最低润色强度）的可视化，而非新增后端语义。**保存状态改为对用户完全不可见**（取消光晕与控件禁用），取代上一轮“以节点光晕表达保存中”的表述，理由是禁用控件会打断用户连续操作。**滚动切换时长定为 380ms**，超出原先建议的 120–160ms 区间，因为在该区间内方向感来不及被察觉。以上仅涉及设置界面与其交互反馈，不改变四档后端语义、Prompt 或模型处理策略，因此仍不新增 ADR。
