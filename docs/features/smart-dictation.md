---
{
  ...
  "decisions": ["ADR-0002", "ADR-0003", "ADR-0004", "ADR-0008", "ADR-0012", "ADR-0013"],
  "validationSlices": [
...
- “使用目标场景”不授权默认读取屏幕、文档正文、聊天历史、源代码或剪贴板内容；若未来需要这些上下文，必须形成单独的知情授权和数据边界。
...
- `ASM-SD-06`（Resolved at product level）：办公和 vibe coding 成稿允许受控换行，后续必须调整 Delivery 校验和默认 Windows 注入器的换行语义；除允许的换行表示外，NUL、双向覆盖/隔离符和其他控制字符仍不因此获得授权。
